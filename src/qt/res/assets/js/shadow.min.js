...
function confirmConversationOpenLink() {
	return confirm("您确定打开该链接?\n\n它会泄漏你的IP地址和其他浏览器的元数据，至少我们可以建议你复制链接，打开它.\n\n 您可以选项中禁用他.")
}
...
function deleteMessages(e, t) {
	var a = contacts[e];
	if (!confirm("您确定删除 " + (void 0 == t ? "这些消息?": "这条消息?"))) return ! 1;
	var n = $("#message-count");
	parseInt(n.text());
...
function signMessage() {
	...
	"" !== a ? ($("#sign-result").removeClass("green"), $("#sign-result").addClass("red"), $("#sign-result").html(a), !1) : ($("#sign-signature").val(s.signed_signature), $("#sign-result").removeClass("red"), $("#sign-result").addClass("green"), $("#sign-result").html("消息签名成功"), void 0)
}
function verifyMessage() {
	...
	return a = s.error_msg,
	"" !== a ? ($("#verify-result").removeClass("green"), $("#verify-result").addClass("red"), $("#verify-result").html(a), !1) : ($("#verify-result").removeClass("red"), $("#verify-result").addClass("green"), $("#verify-result").html("消息签名成功"), void 0)
}
...
overviewPage = {
	...
	updateTransaction: function(e) {
		var t = function(e) {
            var temp = "input"==e.t?"接收":"output"==e.t?"发送":"inout"==e.t?"挖矿":"利息";
            if(e.t == "mined"){
                temp = "挖矿";
            }else if(e.t == "stake"){
                temp = "利息";
...
walletManagementPage = {
	...
	compareMnemonics: function() {
		...
		return e == t ? ($("#validate-key-mnemonic").removeClass("red"), $("#validate-key-mnemonic").val(""), !0) : ($("#validate-key-mnemonic").addClass("red"), alert("您提供的恢复短语与生成的恢复短语不匹配-请返回并检查以确保您正确地复制了它"), !1)
	},
	gotoPage: function(e) {
	...
	setMaster: function() {
		...
		return e.length ? (selected = $("#extkey-table tr.selected").attr("data-value").trim(), void 0 === selected || "" === selected ? (alert("请选择一个主要的key."), !1) : (result = bridge.extKeySetMaster(selected), "" !== result.error_msg ? (alert(result.error_msg), !1) : void walletManagementPage.updateKeyList())) : (alert("请选择一个主要的key."), !1)
	},
	setDefault: function() {
		var e = $("#extkey-account-table tr.selected");
		return e.length ? (selected = $("#extkey-account-table tr.selected").attr("data-value").trim(), void 0 === selected || "" === selected ? (alert("请选择一个默认账户"), !1) : (result = bridge.extKeySetDefault(selected), "" !== result.error_msg ? (alert(result.error_msg), !1) : void walletManagementPage.updateAccountList())) : (alert("请选择一个默认账户"), !1)
	},
	changeActiveFlag: function() {
		var e = !1,
		t = $("#extkey-account-table tr.selected"),
		a = $("#extkey-table tr.selected");
		return t.length || a.length ? (t.length ? (selected = t.attr("data-value").trim(), active = t.attr("active-flag").trim(), e = !0) : (selected = a.attr("data-value").trim(), active = a.attr("active-flag").trim()), void 0 === selected || "" === selected ? (alert("请选择一个账户或者Key来来改变激活状态"), !1) : (result = bridge.extKeySetActive(selected, active), "" !== result.error_msg ? (alert(result.error_msg), !1) : void(e ? walletManagementPage.updateAccountList() : walletManagementPage.updateKeyList()))) : (alert("请选择一个账户或者Key来来改变激活状态"), !1)
	}
};
