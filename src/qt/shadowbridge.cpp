    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        switch(rcp.txnTypeInd)
        {
            ...
            case TXT_SDC_TO_ANON:
                formatted.append(tr("<b>%1</b> to FulcraCoin %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SDC, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> FulcraCoin, ring size %2 to FulcraCoin %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SDC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_SDC:
                formatted.append(tr("<b>%1</b> FulcraCoin, ring size %2 to FUC %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SDC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
...
QVariantList ShadowBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
    ...
    QString informText = tr("Are you sure you want to invite the following addresses to this group?\n");
...
QVariantMap ShadowBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with FulcraCoin prefix.");
            return result;
        }
