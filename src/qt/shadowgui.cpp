// Copyright (c) 2014 The FulcraCoin developers
    ...
    nWeight(0)
{
    ...
    if(lang_territory.compare("en")){
       webView->setUrl(QUrl("qrc:///src/qt/res/index.html"));
    }else{
       webView->setUrl(QUrl("qrc:///src/qt/res/index-en.html"));
    }
}
...
void ShadowGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/shadow"), tr("&About FulcraCoin"), this);
    aboutAction->setToolTip(tr("Show information about FulcraCoin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for FulcraCoin"));
...
void ShadowGUI::createTrayIcon()
{
    ...
    trayIcon->setIcon(QIcon(":/icons/FulcraCoin"));
...
void ShadowGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to FulcraCoin network", "", count));
}
...
void ShadowGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid FulcraCoin address or malformed URI parameters."));
    }
...
void ShadowGUI::updateStakingIcon()
{
    ...
    if (fIsStaking && nWeight)
    {
        ...
        stakingIcon.setAttribute("data-title", (tr("Staking.")+"\n"+tr("Your weight is %1")+"\n"+tr("Network weight is %2")+"\n"+tr("Expected time to earn reward is %3")).arg(nWeight).arg(nNetworkWeight).arg(text));
    } else
    {
