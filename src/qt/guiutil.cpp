...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("FulcraCoin"))
        return false;
...
{
    // Convert FulcraCoin:// to FulcraCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("FulcraCoin://"))
    {
        uri.replace(0, 12, "FulcraCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "FulcraCoin.lnk";
}
...
{
    return GetAutostartDir() / "FulcraCoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        optionFile << "[Desktop Entry]\n" \
                   ...
                   << "MimeType=x-scheme-handler/FulcraCoin;\n";
        optionFile.close();
    }
    return true;
}
